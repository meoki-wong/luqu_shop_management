...
export default function Home() {
  ...
  const overOrder = async (item) => {
    setLoading(true);
    try {
      const order = await request.post(`/Orders/OrderFinish/${item.ID}`);
      setLoading(false);
      if (order.statusCode === 200) {
        message.success('该订单已完成');
        getOrders();
      } else {
        message.error('订单处理异常，请稍后重试');
      }
    } catch (err) {
  ...
  const getOrderList = async (type) => {
    ...
    let orderList = [];
    if (UserId == null) {
      const UserInfo = await request.get('/Account/Get');
    ...
    if (roleType == BASE.SHOP_ROLE) {
      ...
      const ShopId = res.data.Data.list[0].ID;
    ...
    for (const i in orderList) {
      ...
      if (orderList[i].PickupCode && orderList[i].PickupCode !== 'null') {
        const PickupCode = JSON.parse(orderList[i].PickupCode);
        // console.log(PickupCode,'序列化之后的车牌好')
        orderList[i].PickupNumber = PickupCode.numberPlate;
        orderList[i].PickupCodeStr = `${PickupCode.numberPlate}【${PickupCode.brand}${PickupCode.color}】`;
      }
      // 把订单状态显示中文
      for (const j in statusCN) {
      ...
      <div className="tab-box">
        <div className="left">
          {tabList.map((item, index) => {
            return (
              <span key={item.value} onClick={() => changeTab(item)} className={item.value == tabIndex ? 'tab-item tab_active' : 'tab-item'}>
                {item.label}
              </span>
            );
          })}
        </div>
        <div className="right">【{userInfo.NickName || '--'}】 自动打印中...</div>
      </div>
      {orderLists.map((item) => {
        return (
          <div key={item.ID}>
            <div className="order-box" key={item.ID}>
              <div className="order-info">
                ...
                <div className="right">
                  <p>
                    <span className="order-tag">包装费</span> <span>￥{0}</span>
                  </p>
                  <p>
                    <span className="order-tag">团券</span> <span>￥{0}</span>
                  </p>
                  <p>
                    <span className="order-tag">店铺满减</span> <span>￥{0}</span>
                  </p>
                </div>
              </div>
              <div className="customer-box">
                <div className="left">
                  <p className="left-info">
                    <span className="iconfont icon-xiaoqiche" />
                    尾号{JSON.parse(item.PickupCode).numberPlate}【{JSON.parse(item.PickupCode).brand}，{JSON.parse(item.PickupCode).color}】
                  </p>
                  <p className="left-info">
                    <span className="iconfont icon-daohang1" />
                    车主：最快预到
                    {Math.floor((new Date(item.ArriveDateTime).getTime() - new Date().getTime()) / 1000 / 60)}分钟【仅代表下单距离】
                  </p>
                  <p className="left-info">
                    <span className="iconfont icon-yonghuNot" />
                    {item.CustomorPhoneNumber}
                  </p>
                </div>
                <div className="right">
                  <span className="title">合计</span>￥{item.TotalPrices}
                </div>
              </div>
            ...
            {item.OState == 'WaitReady' && (
              <div className="order-btn">
                <div className="over-order" onClick={() => overOrder(item)}>
